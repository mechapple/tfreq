...
// Parallelization may be achieved with data parallelism and
// parallel FFTW routines.
// 
// Usage:
// Simply compile:
// :> make all
...
main(int argc, char **argv)
{
   ...
   printf
       ("# Data starts at time step %d and ends at step %d, totalling %d steps.\n#\n",
        tsstart, tsstop, M);
   // 
   ...
   for (i = 0; i < M; i++) {
      ...
      for (j = 0; j < nat; j++)
         v[i][j] = (double *) malloc(3 * sizeof(double));
   }
